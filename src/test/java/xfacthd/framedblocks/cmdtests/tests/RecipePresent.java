...
public final class RecipePresent
{
    ...
    public static int checkForRecipePresence(CommandContext<CommandSourceStack> ctx)
    {
        ...
        if (size > 0)
        {
            ...
            resultMsg = Component.literal("Found %d missing recipes. ".formatted(size)).append(exportMsg);
            color = ChatFormatting.DARK_RED;
        }

        resultMsg = Component.literal(RESULT_MSG.formatted(count.intValue(), time))
                .withStyle(color)
                .append(resultMsg);
