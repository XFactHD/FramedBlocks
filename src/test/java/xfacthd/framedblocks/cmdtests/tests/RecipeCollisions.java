...
public final class RecipeCollisions
{
    ...
    public static void checkForRecipeCollisions(CommandContext<CommandSourceStack> ctx, Consumer<Component> msgQueueAppender) throws CommandSyntaxException
    {
        Level level = ctx.getSource().getLevel();
        Player player = Objects.requireNonNull(ctx.getSource().getPlayerOrException());
        ...
        recipes.forEach(recipe ->
        {
            ...
            recipes.stream()
                    ...
                    .forEach(other ->
                    {
                        ...
                        if (value % 1000000 == 0)
                        {
                            msgQueueAppender.accept(new TextComponent(PROGRESS_MSG.formatted(value)));
                        }
                    });
        });
        ...
        Component resultMsg = new TextComponent("No collisions found");
        ...
        if (!collisions.isEmpty())
        {
            ...
            resultMsg = new TextComponent("Found %d collisions. ".formatted(collisions.size())).append(exportMsg);
