...
public final class ModelBasedOutlineRenderer implements SimpleOutlineRenderer {
    ...
    public void draw(BlockState state, LineDrawer drawer) {
        float[] vertices = cachedVertices.computeIfAbsent(
                stateMerger.apply(state),
                ModelBasedOutlineRenderer::computeVertices
        );
    ...
    private static float[] computeVertices(BlockState state) {
        ...
        model.collectParts(BlockAndTintGetter.EMPTY, BlockPos.ZERO, state, RANDOM, SCRATCH_LIST);
        ...
        for (BlockStateModelPart part : SCRATCH_LIST) {
        ...
        do {
            ...
            outer: for (Line lineOne : lines) {
                for (Line lineTwo : lines) {
                    ...
                    if (!lineOne.isParallelTo(lineTwo) || !check.test(lineOne, lineTwo, shared)) {
                        continue;
                    }
    ...
    private static void removeOverlappingSegments(Set<Line> lines) {
        ...
        do {
            ...
            outer: for (Line lineOne : lines) {
                for (Line lineTwo : lines) {
                    if (lineOne == lineTwo || !lineOne.isParallelTo(lineTwo)) {
                        continue;
                    }
                    if (!lineOne.contains(lineTwo.x1, lineTwo.y1, lineTwo.z1)) {
                        continue;
                    }
                    if (!lineOne.contains(lineTwo.x2, lineOne.y2, lineTwo.z2)) {
                        continue;
                    }
    ...
    private static <T> @Nullable Integer decrementAndEliminate(T key, @Nullable Integer value) {
        if (value == null) {
            return null;
        }
