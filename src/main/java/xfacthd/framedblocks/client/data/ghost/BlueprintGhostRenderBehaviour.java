...
public class BlueprintGhostRenderBehaviour implements GhostRenderBehaviour
{
    ...
    public CamoPair readCamo(ItemStack stack, ItemStack proxiedStack, boolean secondPass)
    {
        ...
        if (stack.hasTag() && stack.getTag().contains("blueprint_data"))
        {
            ...
            Set<Pair<BlockState, ItemStack>> camos = FramedBlueprintItem.getCamos((BlockItem) proxiedStack.getItem(), tag);

            Iterator<Pair<BlockState, ItemStack>> it = camos.iterator();
            BlockState camoState = it.next().getFirst();
            BlockState camoStateTwo = null;
            if (it.hasNext())
            {
                camoStateTwo = it.next().getFirst();
            }
    ...
    @Override
    public IModelData appendModelData(ItemStack stack, ItemStack proxiedStack, BlockPlaceContext ctx, BlockState renderState, boolean secondPass, IModelData data)
    {
