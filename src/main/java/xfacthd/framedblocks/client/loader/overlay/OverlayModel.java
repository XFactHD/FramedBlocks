...
class OverlayModel extends BakedModelWrapper<BakedModel>
{
    ...
    {
        return untranslateQuads(model.getQuads(null, null, random, EmptyModelData.INSTANCE), offset, scale);
    }
    ...
    private static Map<Direction, List<BakedQuad>> makeCulledQuads(BakedModel model, Vector3f offset, Vector3f scale, Random random)
    {
        ...
        {
            List<BakedQuad> quads = model.getQuads(null, side, random, EmptyModelData.INSTANCE);
