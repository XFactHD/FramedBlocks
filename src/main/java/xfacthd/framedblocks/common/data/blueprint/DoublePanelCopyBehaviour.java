...
public class DoublePanelCopyBehaviour extends DoubleBlockCopyBehaviour
{
    ...
    public void postProcessPaste(Level level, BlockPos pos, Player player, CompoundTag blueprintData, ItemStack dummyStack)
    {
        ...
        if (state.getValue(FramedProperties.FACING_NE) != player.getDirection())
        {
            ...
            {
                //Mirror camos
                ItemStack camoStackOne = be.getCamoStack();
                ItemStack camoStackTwo = be.getCamoStackTwo();
                BlockState camoOne = be.getCamoState();
                BlockState camoTwo = be.getCamoStateTwo();

                be.setCamo(camoStackTwo, camoTwo, false);
                be.setCamo(camoStackOne, camoOne, true);
            }
        }
    }
}
