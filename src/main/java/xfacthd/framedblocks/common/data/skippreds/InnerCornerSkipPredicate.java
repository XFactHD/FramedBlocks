...
public class InnerCornerSkipPredicate implements SideSkipPredicate
{
    ...
    public boolean test(IBlockReader world, BlockPos pos, BlockState state, BlockState adjState, Direction side)
    {
        ...
        switch (adjBlock)
        {
            ...
            case FRAMED_THREEWAY_CORNER: return testAgainstThreewaySlope(world, pos, dir, type, adjState, side);
            case FRAMED_INNER_PRISM_CORNER:
            case FRAMED_INNER_THREEWAY_CORNER: return testAgainstInnerThreewaySlope(world, pos, dir, type, adjBlock, adjState, side);
    ...
    private boolean testAgainstInnerCorner(IBlockReader world, BlockPos pos, Direction dir, CornerType type, BlockState adjState, Direction side)
    {
        ...
        if (!type.isHorizontal() && adjType == type && ((side == dir.getOpposite() && adjDir == dir.getClockWise()) || (side == dir.getCounterClockWise() && adjDir == dir.getCounterClockWise())))
        {
        ...
        else if (type.isHorizontal() && type.isHorizontalAdjacentInner(dir, side, adjType) && adjDir == dir)
        {
            return SideSkipPredicate.compareState(world, pos, side);
                ...
                ((side == dir.getOpposite() && adjType.isRight() && adjDir == dir.getClockWise()) || (side == dir.getCounterClockWise() && !adjType.isRight() && adjDir == dir))
        )
        {
            return SideSkipPredicate.compareState(world, pos, side);
        }
        else if (type.isHorizontal() && !adjType.isHorizontal() && adjType.isTop() == type.isTop() &&
                ((!type.isRight() && side == dir.getClockWise() && adjDir == dir) || (type.isRight() && side == dir.getCounterClockWise() && adjDir == dir.getCounterClockWise()))
        )
        {
            return SideSkipPredicate.compareState(world, pos, side);
        }
    ...
    private boolean testAgainstSlope(IBlockReader world, BlockPos pos, Direction dir, CornerType type, BlockState adjState, Direction side)
    {
        ...
        if (!type.isHorizontal() && adjType != SlopeType.HORIZONTAL && ((side == dir.getOpposite() && adjDir == dir.getClockWise()) || (side == adjDir.getCounterClockWise() && adjDir == dir)))
        {
        ...
        else if (type.isHorizontal())
        {
            ...
            {
                return ((!type.isRight() && side == dir.getClockWise()) || (type.isRight() && side == dir.getCounterClockWise())) && SideSkipPredicate.compareState(world, pos, side);
            }
        }
        return false;
    }
    ...
    private boolean testAgainstDoubleSlope(IBlockReader world, BlockPos pos, Direction dir, CornerType type, BlockState adjState, Direction side)
    {
        ...
        else if (type.isHorizontal() && adjType != SlopeType.HORIZONTAL && ((side == dir.getClockWise() && !type.isRight()) || (side == dir.getCounterClockWise() && type.isRight())))
        {
            Direction face = type.isTop() == adjTop ? adjDir : adjDir.getOpposite();
    ...
    private boolean testAgainstInnerThreewaySlope(IBlockReader world, BlockPos pos, Direction dir, CornerType type, BlockType adjBlock, BlockState adjState, Direction side)
    {
        ...
        if (!type.isHorizontal() && adjTop == type.isTop() && ((side == dir.getOpposite() && adjDir == dir.getOpposite()) || (side == dir.getCounterClockWise() && adjDir == dir)))
        {
            return SideSkipPredicate.compareState(world, pos, side);
        }
        else if (type.isHorizontal())
        ...
        else
        {
            ...
            ))
            {
                return SideSkipPredicate.compareState(world, pos, side, dir);
                     ...
                     ((!type.isRight() && (adjDir == dir || adjDir == dir.getOpposite())) || (type.isRight() && (adjDir == dir.getClockWise() || adjDir == dir.getCounterClockWise())))
            )
            {
                return SideSkipPredicate.compareState(world, pos, side, dir);
            }
