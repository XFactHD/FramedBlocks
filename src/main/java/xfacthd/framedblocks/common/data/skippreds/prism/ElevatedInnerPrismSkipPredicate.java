...
public final class ElevatedInnerPrismSkipPredicate implements SideSkipPredicate
{
    ...
    public boolean test(BlockGetter level, BlockPos pos, BlockState state, BlockState adjState, Direction side)
    {
        ...
        if (adjState.getBlock() instanceof IFramedBlock block && block.getBlockType() instanceof BlockType type)
        {
            return switch (type)
            {
                case FRAMED_ELEVATED_INNER_PRISM -> testAgainstInnerPrism(
                        dirAxis, adjState, side
                );
                case FRAMED_ELEVATED_INNER_SLOPED_PRISM -> testAgainstInnerSlopedPrism(
                        dirAxis, adjState, side
                );
                default -> false;
            };
    ...
    private static boolean testAgainstInnerSlopedPrism(DirectionAxis dirAxis, BlockState adjState, Direction side)
    {
        ...
        return getTriDir(dirAxis, side).isEqualTo(ElevatedInnerSlopedPrismSkipPredicate.getTriDir(adjCmpDir, side.getOpposite()));
    }
