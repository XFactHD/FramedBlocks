...
public interface GhostRenderBehaviour
{
     ...
     */
    @Nullable
    default ItemStack getProxiedStack(ItemStack stack) { return null; }
     ...
     */
    default boolean hasSecondBlock(ItemStack stack, @Nullable ItemStack proxiedStack) { return false; }
     ...
     * @param ctx The {@link BlockPlaceContext} to use for determining the resulting {@code BlockState} via {@link Block#getStateForPlacement(BlockPlaceContext)}
     * @param hitState The {@code BlockState} hit by the given {@code BlockHitResult}
     * @param secondPass True if {@link GhostRenderBehaviour#hasSecondBlock(ItemStack, ItemStack)} returns true and the second block is being rendered, otherwise false
     * @return The {@code BlockState} to render or null when none could be determined
     */
    @Nullable
    default BlockState getRenderState(ItemStack stack, @Nullable ItemStack proxiedStack, BlockHitResult hit, BlockPlaceContext ctx, BlockState hitState, boolean secondPass)
    {
     ...
     */
    default BlockPos getRenderPos(ItemStack stack, @Nullable ItemStack proxiedStack, BlockHitResult hit, BlockPlaceContext ctx, BlockState hitState, BlockPos defaultPos, boolean secondPass)
    {
        return defaultPos;
    }
    ...
    default boolean canRenderAt(ItemStack stack, @Nullable ItemStack proxiedStack, BlockHitResult hit, BlockPlaceContext ctx, BlockState hitState, BlockState renderState, BlockPos renderPos)
    {
    ...
    default CamoPair readCamo(ItemStack stack, @Nullable ItemStack proxiedStack, boolean secondPass)
    {
        ...
        if (stack.hasTag() && stack.getTag().contains("BlockEntityTag"))
        {
            ...
            return new CamoPair(camoState, Blocks.AIR.defaultBlockState());
        }
     ...
     */
    default CamoPair postProcessCamo(ItemStack stack, @Nullable ItemStack proxiedStack, BlockPlaceContext ctx, BlockState renderState, boolean secondPass, CamoPair camo)
    {
        return camo;
    }

    /**
     * Append any additional data apart from the camos to the given {@link IModelData}.
     * @param stack The {@link ItemStack} in the players main hand
     * @param proxiedStack The proxied {@code ItemStack} as returned from {@link GhostRenderBehaviour#getProxiedStack(ItemStack)}
     * @param ctx The {@link BlockPlaceContext} to use for determining the resulting
     * @param renderState The {@code BlockState} to render
     * @param secondPass True if {@link GhostRenderBehaviour#hasSecondBlock(ItemStack, ItemStack)} returns true and the second block is being rendered, otherwise false
     * @param data The prepared {@code IModelData} to be given to the {@link BakedModel} that is to be rendered
     * @return The {@code IModelData} with any necessary modifications applied to it
     */
    default IModelData appendModelData(ItemStack stack, @Nullable ItemStack proxiedStack, BlockPlaceContext ctx, BlockState renderState, boolean secondPass, IModelData data)
    {
        return data;
    }
}
